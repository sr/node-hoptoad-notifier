...
var Hoptoad = {
  ...
  notify: function(error) {
    if (Hoptoad.ENVIRONMENT === undefined) {
      Hoptoad.environment = process.env['NODE_ENV'] || 'production';
    }
  ...
  generateBacktrace: function(error) {
    if (error === undefined || typeof error.stack != 'string') {
      return [];
    }
  ...
  generateXML: function(error) {
    ...
    return xml.replace('PROJECT_ROOT',      process.cwd())
              .replace('EXCEPTION_CLASS',   error.type || 'Error')
              .replace('EXCEPTION_MESSAGE', error.message || 'Unknown error.')
